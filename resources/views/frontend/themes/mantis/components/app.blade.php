<main class="flex-1 border border-gray-200 dark:border-gray-700 m-5 md:m-0" x-data="{ show: false, id: 0 }">
    @if ($error)
        <div id="imap-error" class="flex items-center w-full h-full fixed top-0 left-0 bg-red-900 dark:bg-red-800 opacity-75 dark:opacity-80 z-50">
            <div class="flex flex-col mx-auto">
                ...
                <div class="p-2 mx-auto bg-red-800 dark:bg-red-700 text-white dark:text-gray-100 leading-none flex lg:inline-flex" role="alert">
                    <span class="flex bg-red-500 dark:bg-red-600 uppercase px-2 py-1 text-xs font-bold mr-3">{{ __("Error") }}</span>
    ...
    @if ($messages)
        <div class="mailbox">
            <div x-show="!show" class="list">
                <div class="head flex items-center gap-3 pt-5 pb-6 px-7 bg-gray-700 text-white dark:text-gray-100">
            ...
            <div x-show="show" class="message">
                <div class="head flex items-center bg-gray-700 text-white dark:text-gray-100 py-5 px-7">
                ...
                @foreach ($messages as $message)
                    <div x-show="id === {{ $message["id"] }}" id="message-{{ $message["id"] }}" class="message min-h-tm-mantis-message">
                        ...
                        <div class="text-wrap py-4 px-7 bg-white dark:bg-gray-800">
                            <iframe class="w-full flex flex-grow min-h-tm-mantis-iframe" srcdoc="{{ $message["content"] }}" frameborder="0"></iframe>
                            @if (count($message["attachments"]) > 0)
                                ...
                                <div class="flex pb-5 px-6">
                                    @foreach ($message["attachments"] as $attachment)
                                        <a class="py-2 px-3 mr-3 text-sm border-2 border-black dark:border-gray-300 hover:bg-black dark:hover:bg-gray-700 hover:text-white dark:hover:text-gray-100 text-gray-900 dark:text-gray-100" href="{{ $attachment["url"] }}" download>
    ...
    @else
        <div id="empty-inbox" class="flex items-center">
            <div class="flex-1 flex flex-col justify-center items-center text-gray-500 dark:text-gray-400 h-60">
